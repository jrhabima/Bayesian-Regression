...
```{r trace-plot1, fig.cap="Trace plot for model parameters", echo = FALSE, warning=FALSE}
colors <- c("red",  "blue", "green", "purple", "black", "orange")

plot(out_lm$sigma2, type = "l" , main = "True error variance vs estimated error variance", cex.main=1)
abline(h = truesigma2, col = "red")

matplot(out_lm$beta, type = "l",  xlab = "iterations", ylab = "beta values",
        col = colors, main = "True beta vs estimated beta", cex.main=1)
abline(h = truebeta, col = colors)

plot(truebeta, apply(out_lm$beta, 2, mean), ylab = "beta post mean", xlab = "true beta",
     main  = "true beta vs beta posterior mean ")
abline(0, 1, col = "red")
```
...
```{r tuning-choices, fig.cap="Trace plots resulting from different tuning parameters"}
knitr::include_graphics(here::here("figures", "MH_tuning_choices.png"))
```
...
if(!file.exists(here::here("results", "adapt__mh_lm_out10.RData"))){
    ...
    adapt__mh_lm_out10 <- Adapt_MH_MCMC_lm(X = X, y = y, K = 10000, 
...
```{r, echo = FALSE, warning=FALSE, message=FALSE}
# out_coda_mh <- as.mcmc(convert_to_coda_single_chain(adapt__mh_lm_out))
# out_coda_mh <- tidy_draws(out_coda_mh) ## convert to tidybay
```

### Trace plots

```{r trace-plot_mh, fig.cap="Trace plot for model parameters", echo = FALSE, warning=FALSE}
...
### MH regression model diagnostics

Consider the same number of iteration as in Gibbs sampling, $N = 10,000$
...
plot(resid_mh10, y_pred_mh100, main = "resuduals vs fitted values")
